...
<AccordionGroup>
  ...
  <Accordion title="Resolution proposers whitelist" icon="user-check">
    The creator wallet also needs an on-chain proposer whitelist before creating markets. This controls who can propose outcomes for that creator's markets on UMA.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Resolution proposers whitelist" icon="user-check">
    The flow checks whether the connected creator wallet already has a proposer whitelist. If it does not, click **Create whitelist** and sign with the creator wallet before continuing.
  </Accordion>
...
Use **Admin > Events > Calendar > Proposers** to manage the wallets allowed to propose outcomes on UMA.
...
<Steps>
  ...
  <Step title="Add proposer wallets">
    Add the trusted wallets that will propose UMA outcomes for this creator. You can add or remove proposers later; open unresolved markets use the current on-chain list.
  </Step>
</Steps>
...
<Info>
Proposers still need their own funds when using UMA: USDC for bond/final fee and POL for gas. In the current setup this is usually around `$750` in USDC locked as bond/final fee, with a `$5` reward per market when the answer is accepted as correct.
</Info>
