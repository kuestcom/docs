---
title: General
description: Configure the brand, market context, featured markets, synced sources, Terms of Service, and geoblocking in Admin -> General.
icon: settings
---

<Info>
Start here after deployment. Configure the public brand and support details first, then review discovery, market context, creator sources, Terms of Service, and geoblocking.
</Info>

<Tip>
Save one section at a time and verify the public site in another session. This makes incorrect branding, links, or visibility rules easier to isolate.
</Tip>
...
<AccordionGroup>
  <Accordion title="Enable market context" icon="toggle-right" defaultOpen>
    Turn AI-generated context on or off on event pages. This feature requires an OpenRouter API key and model configured in [Integrations](/admin/integrations#openrouter).
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Synced market sources" icon="users">
    Manage trusted market creators. Add another Kuest site URL to import its creator wallets automatically, or add one wallet with a display name. These sources also determine which creator wallets pass the Create Event pre-sign check.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Terms of Service" icon="file-text" defaultOpen>
    Use this section to update the Terms of Service shown on your site. Use the flag selector next to <strong>Choose the language</strong> to switch between the available versions and edit the terms for each language.

    Use Markdown formatting to edit the content: `#` for titles, `##` for headings, `-` for bullet lists, blank lines for paragraphs, and `**text**` for bold text. You can also use `{{siteName}}`, `{{siteNameUpper}}`, and `{{siteUrl}}` as site variables.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Blocked countries" icon="map-pin-off">
