...
| `POLYGON_RPC_URL` | Overrides the default RPC endpoint used for on-chain reads in the app. | Set it to use a private or higher-performance RPC provider, or when the default network RPC is unavailable or rate-limited. | `https://polygon-bor-rpc.publicnode.com` |
