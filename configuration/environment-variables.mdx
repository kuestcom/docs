...
| `NEXT_PUBLIC_RPC_URL` | Overrides the default RPC endpoint used for on-chain reads in the app. | Set it when the default network RPC is unavailable, rate-limited, or when your deployment should use a specific browser-safe RPC provider. | `https://polygon-bor-rpc.publicnode.com` |
