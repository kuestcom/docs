...
<Info>
`pnpm db:push` configures the supplied database scheduler only when both `pg_cron` and `pg_net` are available. Read the migration output: if either extension is unavailable, use the external schedule below.
</Info>
...
When database scheduling is available, the migration also creates `clean-cron-details` at `0 0 * * *` and `clean-jobs` at `15 * * * *` as maintenance jobs. External schedulers should only call the HTTP sync endpoints above.
