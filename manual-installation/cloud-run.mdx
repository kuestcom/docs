...
The supplied Cloud Build file builds and deploys the application, but it does not run database migrations. Run `pnpm db:push` from a secured environment with the production `POSTGRES_URL`, `SITE_URL`, and `CRON_SECRET` before opening traffic.

After deployment, verify the latest revision is serving successfully and open the canonical `SITE_URL`. If the database does not have both `pg_cron` and `pg_net` with the supplied jobs active, configure [Scheduled Jobs](/manual-installation/scheduler-jobs).

## Scheduler

If the supplied database jobs are not active, create Cloud Scheduler jobs that follow [Scheduled Jobs](/manual-installation/scheduler-jobs).
