...
<Steps>
  ...
  <Step title="Point Railway to the config file">
    ...
    - Pre-deploy command: `pnpm db:push`
  </Step>
  ...
  <Step title="Deploy and verify migrations">
    Trigger a deployment and confirm the pre-deploy step runs `pnpm db:push` successfully in the Railway deploy logs before opening production traffic.
  </Step>
</Steps>

## Scheduler

Railway deployment does not replace the sync scheduler requirement. If the supplied database jobs are not active, configure external jobs following [Scheduled Jobs](/manual-installation/scheduler-jobs).
