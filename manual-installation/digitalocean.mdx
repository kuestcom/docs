...
<Steps>
  ...
  <Step title="Create resources and validate">
    Deploy the app and wait for build completion. Run `pnpm db:push` from a secured environment connected to the production database, then validate the main route.
  </Step>
...
App Platform deployment does not replace the sync scheduler requirement. If the supplied database jobs are not active, use an external scheduler that follows [Scheduled Jobs](/manual-installation/scheduler-jobs).
